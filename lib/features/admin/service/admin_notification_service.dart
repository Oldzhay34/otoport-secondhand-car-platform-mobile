...
class AdminNotificationService {
    ...
    _dio.interceptors.add(
      InterceptorsWrapper(
        onRequest: (options, handler) async {
          ...
          debugPrint('ADMIN NOTIF REQUEST => ${options.method} ${options.path}');
          handler.next(options);
        },
        onResponse: (response, handler) {
          debugPrint(
            'ADMIN NOTIF RESPONSE <= ${response.requestOptions.path} '
        ...
        onError: (error, handler) async {
          debugPrint(
            'ADMIN NOTIF ERROR <= ${error.requestOptions.path} '
  ...
  Future<List<AdminStoreOptionDto>> getStores() async {
    await _ensureAdminRole();

    final response = await _dio.get('/api/admin/notifications/stores');

    final rawList = (response.data as List?) ?? const [];

    return rawList
        .map(
          (e) => AdminStoreOptionDto.fromJson(
  ...
  Future<void> create(AdminNotificationCreateRequest request) async {
    await _ensureAdminRole();
