...
class AdminStoreAccountService {
    ...
    _dio.interceptors.add(
      InterceptorsWrapper(
        onRequest: (options, handler) async {
          ...
          debugPrint('ADMIN STORE ACCOUNT REQUEST => ${options.method} ${options.path}');
          handler.next(options);
        },
        onResponse: (response, handler) {
          debugPrint(
            'ADMIN STORE ACCOUNT RESPONSE <= ${response.requestOptions.path} '
        ...
        onError: (error, handler) async {
          debugPrint(
            'ADMIN STORE ACCOUNT ERROR <= ${error.requestOptions.path} '
  ...
  Future<AdminCreateStoreAccountResponse> createStore(
      AdminCreateStoreAccountRequest request,
      ) async {
    await _ensureAdminRole();

    final response = await _dio.post(
      '/api/admin/store-accounts',
      data: request.toJson(),
    );

    return AdminCreateStoreAccountResponse.fromJson(
