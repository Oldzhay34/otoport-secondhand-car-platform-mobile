...
import 'package:otoport_mobile/features/admin/models/update_store_plan_request.dart';
...
class AdminStoreSubscriptionService {
    ...
    _dio.interceptors.add(
      InterceptorsWrapper(
        onRequest: (options, handler) async {
          ...
          debugPrint('ADMIN SUB REQUEST => ${options.method} ${options.path}');
          handler.next(options);
        },
        onResponse: (response, handler) {
          debugPrint(
            'ADMIN SUB RESPONSE <= ${response.requestOptions.path} '
        ...
        onError: (error, handler) async {
          debugPrint(
            'ADMIN SUB ERROR <= ${error.requestOptions.path} '
  ...
  Future<List<AdminStoreSubscriptionRowDto>> getStores() async {
    await _ensureAdminRole();

    final response = await _dio.get('/api/admin/store-subscriptions/stores');

    final rawList = (response.data as List?) ?? const [];

    return rawList
        .map(
          (e) => AdminStoreSubscriptionRowDto.fromJson(
  ...
  Future<bool> setPlan({
    ...
    final response = await _dio.patch(
      '/api/admin/store-subscriptions/$storeId/plan',
      data: UpdateStorePlanRequest(plan: plan).toJson(),
    );
