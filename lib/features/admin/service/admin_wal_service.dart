...
class AdminWalService {
    ...
    _dio.interceptors.add(
      InterceptorsWrapper(
        onRequest: (options, handler) async {
          ...
          debugPrint('ADMIN WAL REQUEST => ${options.method} ${options.path}');
          handler.next(options);
        },
        onResponse: (response, handler) {
          debugPrint(
            'ADMIN WAL RESPONSE <= ${response.requestOptions.path} '
        ...
        onError: (error, handler) async {
          debugPrint(
            'ADMIN WAL ERROR <= ${error.requestOptions.path} '
  ...
  Future<List<AdminWalRowDto>> getRecent({
    int limit = 100,
    String sort = 'desc',
  }) async {
  ...
  Future<List<AdminWalRowDto>> search(AdminWalSearchRequest request) async {
    await _ensureAdminRole();
