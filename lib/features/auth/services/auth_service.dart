import 'package:dio/dio.dart';
import 'package:flutter/foundation.dart';
import 'package:otoport_mobile/core/constants/api_constants.dart';
import 'package:otoport_mobile/core/storage/token_storage.dart';
import 'package:otoport_mobile/features/auth/models/forgot_password_request.dart';
import 'package:otoport_mobile/features/auth/models/login_request.dart';
import 'package:otoport_mobile/features/auth/models/login_response.dart';
import 'package:otoport_mobile/features/auth/models/me_response.dart';
import 'package:otoport_mobile/features/auth/models/refresh_request.dart';
import 'package:otoport_mobile/features/auth/models/register_request.dart';
import 'package:otoport_mobile/features/auth/models/register_response.dart';
import 'package:otoport_mobile/features/auth/models/reset_password_request.dart';
import 'package:otoport_mobile/features/auth/models/simple_ok_response.dart';
import 'package:otoport_mobile/features/auth/models/verify_email_request.dart';
import 'package:otoport_mobile/features/auth/models/verify_reset_code_request.dart';
import 'package:otoport_mobile/features/auth/models/verify_reset_code_response.dart';

class AuthService {
  late final Dio _dio;
  final TokenStorage _tokenStorage = TokenStorage();

  AuthService() {
    _dio = Dio(
      BaseOptions(
        baseUrl: ApiConstants.baseUrl,
        connectTimeout: const Duration(seconds: 15),
        receiveTimeout: const Duration(seconds: 15),
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
        },
      ),
    );

    _dio.interceptors.add(
      InterceptorsWrapper(
        onRequest: (options, handler) async {
          final token = await _tokenStorage.getAccessToken();

          final isPublicEndpoint =
              options.path == '/api/auth/login' ||
                  options.path == '/api/auth/login-mobile' ||
                  options.path == '/api/auth/register' ||
                  options.path == '/api/auth/verify-email' ||
                  options.path == '/api/auth/refresh-mobile' ||
                  options.path == '/api/auth/password/forgot' ||
                  options.path == '/api/auth/password/verify-code' ||
                  options.path == '/api/auth/password/reset';

          options.headers.remove('Authorization');

          if (!isPublicEndpoint && token != null && token.trim().isNotEmpty) {
            options.headers['Authorization'] = 'Bearer ${token.trim()}';
          }

          debugPrint('REQUEST => ${options.method} ${options.path}');
          debugPrint('AUTH HEADER => ${options.headers['Authorization']}');

          handler.next(options);
        },
        onResponse: (response, handler) {
          debugPrint(
            'RESPONSE <= ${response.requestOptions.path} '
                '[${response.statusCode}] ${response.data}',
          );
          handler.next(response);
        },
        onError: (error, handler) async {
          debugPrint(
            'ERROR <= ${error.requestOptions.path} '
                '[${error.response?.statusCode}] ${error.response?.data}',
          );

          final requestPath = error.requestOptions.path;

          final shouldSkipRefresh =
              requestPath == '/api/auth/login' ||
                  requestPath == '/api/auth/login-mobile' ||
                  requestPath == '/api/auth/register' ||
                  requestPath == '/api/auth/verify-email' ||
                  requestPath == '/api/auth/refresh-mobile' ||
                  requestPath == '/api/auth/password/forgot' ||
                  requestPath == '/api/auth/password/verify-code' ||
                  requestPath == '/api/auth/password/reset';

          if (error.response?.statusCode == 401 && !shouldSkipRefresh) {
            try {
              final refreshToken = await _tokenStorage.getRefreshToken();

              if (refreshToken == null || refreshToken.trim().isEmpty) {
                await _tokenStorage.clearAll();
                return handler.next(error);
              }

              final refreshResponse = await _dio.post(
                '/api/auth/refresh-mobile',
                data: RefreshRequest(
                  refreshToken: refreshToken.trim(),
                ).toJson(),
                options: Options(
                  headers: {
                    'Authorization': null,
                  },
                ),
              );

              final data = Map<String, dynamic>.from(refreshResponse.data);

              final newAccessToken =
              (data['accessToken'] ?? '').toString().trim();
              final newRefreshToken =
              (data['refreshToken'] ?? '').toString().trim();

              if (newAccessToken.isEmpty || newRefreshToken.isEmpty) {
                await _tokenStorage.clearAll();
                return handler.next(error);
              }

              final role = await _tokenStorage.getRole() ?? '';
              final userId = await _tokenStorage.getUserId() ?? '';

              await _tokenStorage.saveTokens(
                accessToken: newAccessToken,
                refreshToken: newRefreshToken,
                role: role,
                userId: userId,
              );

              final retryRequest = error.requestOptions;
              retryRequest.headers['Authorization'] = 'Bearer $newAccessToken';

              final clonedResponse = await _dio.fetch(retryRequest);
              return handler.resolve(clonedResponse);
            } catch (e) {
              debugPrint('REFRESH ERROR => $e');
              await _tokenStorage.clearAll();
              return handler.next(error);
            }
          }

          handler.next(error);
        },
      ),
    );
  }

  Future<LoginResponse> login(LoginRequest request) async {
    final response = await _dio.post(
      '/api/auth/login-mobile',
      data: request.toJson(),
    );

    final data = Map<String, dynamic>.from(response.data);

    final accessToken = (data['accessToken'] ?? '').toString().trim();
    final refreshToken = (data['refreshToken'] ?? '').toString().trim();

    if (accessToken.isEmpty || refreshToken.isEmpty) {
      throw Exception('Login response token içermiyor: $data');
    }

    return LoginResponse.fromJson(data);
  }

  Future<MeResponse> me() async {
    final response = await _dio.get('/api/auth/me');
    return MeResponse.fromJson(Map<String, dynamic>.from(response.data));
  }

  Future<MeResponse> meWithAccessToken(String accessToken) async {
    final cleanToken = accessToken.trim();

    if (cleanToken.isEmpty) {
      throw Exception('Access token boş olduğu için /me çağrılamadı.');
    }

    final response = await _dio.get(
      '/api/auth/me',
      options: Options(
        headers: {
          'Authorization': 'Bearer $cleanToken',
        },
      ),
    );

    return MeResponse.fromJson(Map<String, dynamic>.from(response.data));
  }

  Future<RegisterResponse> register(RegisterRequest request) async {
    final response = await _dio.post(
      '/api/auth/register',
      data: request.toJson(),
    );

    return RegisterResponse.fromJson(
      Map<String, dynamic>.from(response.data),
    );
  }

  Future<Map<String, dynamic>> verifyEmail(VerifyEmailRequest request) async {
    final response = await _dio.post(
      '/api/auth/verify-email',
      data: request.toJson(),
    );

    return Map<String, dynamic>.from(response.data);
  }

  Future<Map<String, dynamic>> refreshMobile(RefreshRequest request) async {
    final response = await _dio.post(
      '/api/auth/refresh-mobile',
      data: request.toJson(),
    );

    return Map<String, dynamic>.from(response.data);
  }

  Future<void> forgotPassword(ForgotPasswordRequest request) async {
    await _dio.post(
      '/api/auth/password/forgot',
      data: request.toJson(),
    );
  }

  Future<VerifyResetCodeResponse> verifyResetCode(
      VerifyResetCodeRequest request,
      ) async {
    final response = await _dio.post(
      '/api/auth/password/verify-code',
      data: request.toJson(),
    );

    return VerifyResetCodeResponse.fromJson(
      Map<String, dynamic>.from(response.data),
    );
  }

  Future<void> resetPassword(ResetPasswordRequest request) async {
    await _dio.post(
      '/api/auth/password/reset',
      data: request.toJson(),
    );
  }

  Future<SimpleOkResponse> logout() async {
    final response = await _dio.post('/api/auth/logout');
    return SimpleOkResponse.fromJson(Map<String, dynamic>.from(response.data));
  }
}