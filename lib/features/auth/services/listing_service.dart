...
class ListingService {
        ...
        onError: (error, handler) async {
          ...
          if (error.response?.statusCode == 401 && !shouldSkipRefresh) {
            try {
              ...
              debugPrint('LISTING REFRESH ERROR => $e');
              await _tokenStorage.clearAll();
              return handler.next(error);
            }
          }
  ...
  Future<ListingDetailModel> getListingDetail(int listingId) async {
    final response = await _dio.get('/api/listings/$listingId/detail');

    return ListingDetailModel.fromJson(
