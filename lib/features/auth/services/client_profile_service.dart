...
import 'package:otoport_mobile/features/auth/models/client_profile_get_response.dart';
import 'package:otoport_mobile/features/auth/models/client_profile_response.dart';
...
class ClientProfileService {
        ...
        onError: (error, handler) async {
          ...
          if (error.response?.statusCode == 401 && !shouldSkipRefresh) {
            try {
              ...
              await _tokenStorage.saveTokens(accessToken: newAccessToken, refreshToken: newRefreshToken, role: role, userId: userId,);
  ...
  Future<ClientProfileGetResponse> getMyProfile() async {
    final response = await _dio.get('/api/client/profile/me');

    return ClientProfileGetResponse.fromJson(
      Map<String, dynamic>.from(response.data),
    );
  }

  Future<ClientProfileResponse> updateMyProfile(
      ClientProfileUpdateRequest request,
      ) async{
    final response= await _dio.put('/api/client/profile/me', data: request.toJson(),);

    final data =response.data;
    if (data ==null) {
      return ClientProfileResponse.success();
    }

    if (data is Map) {
      return ClientProfileResponse.fromJson(
